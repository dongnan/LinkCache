...
class Memcache implements Base, Lock, Incr, Multi {
    ...
    public function set($key, $value, $time = -1) {
        ...
        try {
            if ($time > 0) {
                if ($this->handler->set($key, $value, $this->compress($value), $time <= 2592000 ? $time : time() + $time)) {
                    $this->handler->set(self::timeKey($key), $time + time(), 0, $time <= 2592000 ? $time : time() + $time);
                    return true;
                }
                return false;
            }
    ...
    public function setnx($key, $value, $time = -1) {
        ...
        try {
            if ($time > 0) {
                if ($this->handler->add($key, $value, $this->compress($value), $time <= 2592000 ? $time : time() + $time)) {
    ...
    public function lock($key, $time = 60) {
        try {
            return $this->handler->set(self::lockKey($key), 1, 0, $time <= 2592000 ? $time : time() + $time);
        } catch (Exception $ex) {
            self::exception($ex);
            //连接状态置为false
            $this->isConnected = false;
